...
## Task 021A.6 existing-place water polygons

Task 021A.6 separately authorized water outlines for existing numbered `place.radhakunda` (#1), `place.syamakunda` (#2), and `place.lalitakunda` (#3), using the controlled [three-kuṇḍa evidence](../content/research/radhakunda-three-kunda-controlled-geometries.yaml). Together with Mohana-kuṇḍa, the production map now has four distinct water polygons but still only 72 place records and 71 public numbers. The three numbered places retain their original point coordinates, pin interactions, navigation eligibility, and working-point status; no polygon vertex or display-label location becomes a navigation point.

All four fills/outlines appear only from local zoom 15. Numbered places keep their existing pin/name label and receive no second polygon label; unnumbered Mohana-kuṇḍa keeps its native polygon label. Each water polygon has its own tap selection and selected-outline highlight. The compiled SQLite remains the sole runtime geometry source; research GeoJSON is not loaded by the app. The outlines represent present mapped water only, not historical or nitya-līlā boundaries, public entrances, bathing access, or verified safe approaches. Current edges and public access still require field confirmation. No other Wave A location is released, and Waves B/C remain unauthorized.
